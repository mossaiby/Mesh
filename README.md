...
| Command | Description |
| :--- | :--- |
| **`/status`** | Display active model, router model, tools, MCPs, symbol count, branch, session token usage, USD cost, and context status. |
...
### `models.json`
Defines provider REST endpoints, model configurations, router settings, model tags/descriptions, auto-compaction rules, and global system prompts.
...
| Command | Description |
| :--- | :--- |
| **`/status`** | Display active model, router model, tools, MCPs, symbol count, branch, token metrics display settings, and context status. |
| **`/models`** | List configured models with tags/descriptions (`/models`), discover remote endpoints (`/models discover`), or batch-add models (`/models add openrouter *free*`). |
| **`/switch`** | Switch active model/mode: `/switch auto` (sticky dynamic router), `/switch router [<key>]`, or directly via key (`/switch <key>`). |
| **`/config`** | Toggle system automation and metrics: `/config proxy`, `/config repair`, `/config hooks`, `/config compact`, `/config tokens`, `/config cost`, `/config statistics`. |
| **`/mode`** | Switch operating mode (`/mode build`, `/mode plan`, `/mode review`, `/mode yolo`). |
| **`/guard`** | Configure tool-call safety guard risk assessment (`/guard on`, `/guard mode supervised|autonomous`). |
...
### `models.json`
Defines provider REST endpoints, model configurations, router settings, model tags/descriptions, metrics toggles, auto-compaction rules, and global system prompts.
