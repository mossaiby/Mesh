...
- **Fine-Tuned System Configuration (`/config set`)** — Fine-tune timeouts, token budgets, turn limits, repair retries, and compaction thresholds with single-token commands (e.g. `/config set timeout web 120`, `/config set budget repo-map 800`, `/config set turns agent 8`).
...
| `/mode [plan\|build\|review\|yolo]` | View or switch operating mode: `/mode [plan\|build\|review\|yolo]` |
