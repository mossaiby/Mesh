    ...
    async def cmd_models(self, args):
        ...
        if sub == "add":
            ...
            if len(args) >= 3:
                ...
                if p_key not in self.config_mgr.config.providers:
                    console.print(f"[error]Unknown provider '{p_key}'. Configured providers: {', '.join(self.config_mgr.config.providers.keys())}[/error]")
                    return

                p_cfg = self.config_mgr.config.providers[p_key]
                ...
                configured_keys = set(self.config_mgr.config.models.keys())
                added_count = 0
                skipped_count = 0
                ...
                for m_id in matching_ids:
                    ...
                    self.config_mgr.add_model(model_key, p_key, m_id, context_window=ctx)
            ...
            providers = list(self.config_mgr.config.providers.keys())
            if not providers:
            ...
            if not selected_provider or selected_provider not in self.config_mgr.config.providers:
                return

            p_cfg = self.config_mgr.config.providers[selected_provider]
            ...
            configured_model_ids = {m.model_id for m in self.config_mgr.config.models.values()}
            ...
            try:
                self.config_mgr.add_model(model_key, selected_provider, selected_model_id, context_window=default_ctx)
                ...
                if switch_choice and switch_choice.startswith("Yes"):
                    self.config_mgr.set_active_model(model_key)
            ...
            if target_provider:
                if target_provider in self.config_mgr.config.providers:
                    providers_to_query[target_provider] = self.config_mgr.config.providers[target_provider]
                else:
                    console.print(f"[error]Unknown provider '{target_provider}'. Configured providers: {', '.join(self.config_mgr.config.providers.keys())}[/error]")
                    return
            else:
                providers_to_query = self.config_mgr.config.providers
