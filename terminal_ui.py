    ...
    def get_completions(self, document, complete_event):
        ...
        if cmd0 == "/history":
            if current_arg_index == 1:
                subs = [
                    ("clear", "Clear command history from disk (.mesh/history.txt)"),
                    ("20", "Show last 20 history entries"),
                    ("50", "Show last 50 history entries"),
                    ("100", "Show last 100 history entries"),
                ]
...
class MeshPromptSession:
    ...
    def __init__(self, mesh_instance: Any):
        ...
        if PROMPT_TOOLKIT_AVAILABLE:
            ...
            # Ensure .mesh directory exists for history storage
            try:
    ...
    def clear_history(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.history is not None:
                # Reload / reset in-memory history strings
                self.history._loaded = False
                self.history._loaded_strings = []

            return True, f"Cleared command history from `{HISTORY_FILE}`."
